...
for (i in 1:length(celltype_list)){
  ...
  filename=paste0(celltype,"_up_genes.csv")
...
output_filename=paste0(celltype,"_up_genes.pdf")
pdf(file = output_filename, width = 5, height =5*9/16)
print(p + scale_fill_distiller(palette = "Purples",direction = 1) + scale_colour_manual(values=my_black))
dev.off()
}
}