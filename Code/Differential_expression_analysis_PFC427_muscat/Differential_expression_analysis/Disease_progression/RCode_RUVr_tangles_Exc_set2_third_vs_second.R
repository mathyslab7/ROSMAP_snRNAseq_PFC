...
metadata = metadata %>% mutate(tangles_quartile_numeric =
                                 case_when(tangles_quartile == "second_quarter" ~ 0, 
                                           tangles_quartile == "third_quarter" ~ 1)
)
...
setwd("G:/PFC_429_final_Spring2021/Vanshika/muscat/Revision_Dec2022/tangles_progression/Results_third_vs_second")
...
for (i in 1:length(tbl)){
  ...
  name=gsub("/","_",name)
  k1 <- tbl[[i]]
  write.csv(k1,file=paste0(name,"_","tangles_quartile_numeric_third_vs_second",".csv"))
}
}
##############################################
