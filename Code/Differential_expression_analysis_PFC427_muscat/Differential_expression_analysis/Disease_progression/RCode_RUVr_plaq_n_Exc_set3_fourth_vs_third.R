...
metadata = metadata %>% mutate(plaq_n_quartile_numeric =
                                 case_when(plaq_n_quartile == "third_quarter" ~ 0, 
                                           plaq_n_quartile == "fourth_quarter" ~ 1)
)
...
setwd("G:/PFC_429_final_Spring2021/Vanshika/muscat/Revision_Dec2022/plaq_n_progression/Results_fourth_vs_third")
...
for (i in 1:length(tbl)){
  ...
  name=gsub("/","_",name)
  k1 <- tbl[[i]]
  write.csv(k1,file=paste0(name,"_","plaq_n_quartile_numeric_fourth_vs_third",".csv"))
}
}
##############################################
