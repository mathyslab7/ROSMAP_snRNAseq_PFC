...
Idents(pbmc)<-'nft_quartile'
pbmc=subset(pbmc,idents=c('third_quarter','fourth_quarter'))

#Add numeric coding
metadata = metadata %>% mutate(nft_quartile_numeric =
                                 case_when(nft_quartile == "third_quarter" ~ 0, 
                                           nft_quartile == "fourth_quarter" ~ 1)
)
...
data2=left_join(data,metadata,by="projid")
row.names(data2)=row.names(data)
data2=data2[,c("projid","nft_quartile_numeric")]
pbmc=AddMetaData(pbmc,data2)
...
##############################################
formula = ~nft_quartile_numeric+pmi+age_death+msex
cd = as.data.frame(colData(pb))
cd2=cd[,c("nft_quartile_numeric","pmi","age_death","msex")]
...
nft_quartile_numeric=pb2@colData@listData[["nft_quartile_numeric"]]
set <- newSeqExpressionSet(as.matrix(counts),
                           phenoData = data.frame(nft_quartile_numeric, row.names=pb2@colData@rownames))
set

design <- model.matrix(~nft_quartile_numeric, data=pData(set))
...
#muscat DE analysis
formula = ~nft_quartile_numeric+pmi+age_death+msex+w_1+w_2+w_3+w_4+w_5+w_6+w_7+w_8+w_9+w_10
cd = as.data.frame(colData(pb2))
cd2=cd[,c("nft_quartile_numeric","pmi","age_death","msex","w_1","w_2","w_3","w_4","w_5","w_6","w_7","w_8","w_9","w_10")]
design=model.matrix(formula,cd2)

res <- pbDS(pb2,design = design,coef=as.list(2:4))

#Define output directory
setwd("G:/PFC_429_final_Spring2021/Vanshika/muscat/Revision_Dec2022/nft_progression/Results_fourth_vs_third")

#Indicate name of the variable analyzed
variable="nft_quartile_numeric"
...
for (i in 1:length(tbl)){
  ...
  name=gsub("/","_",name)
  k1 <- tbl[[i]]
  write.csv(k1,file=paste0(name,"_","nft_quartile_numeric_fourth_vs_third",".csv"))
}
}
##############################################
