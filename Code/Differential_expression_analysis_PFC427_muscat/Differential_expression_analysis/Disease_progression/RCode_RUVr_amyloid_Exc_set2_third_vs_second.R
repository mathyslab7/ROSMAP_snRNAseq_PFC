...
metadata = metadata %>% mutate(amyloid_quartile_numeric =
                                 case_when(amyloid_quartile == "second_quarter" ~ 0, 
                                           amyloid_quartile == "third_quarter" ~ 1)
)
...
setwd("G:/PFC_429_final_Spring2021/Vanshika/muscat/Revision_Dec2022/amyloid_progression/Results_third_vs_second")
...
for (i in 1:length(tbl)){
  ...
  name=gsub("/","_",name)
  k1 <- tbl[[i]]
  write.csv(k1,file=paste0(name,"_","amyloid_quartile_numeric_third_vs_second",".csv"))
}
}
##############################################
