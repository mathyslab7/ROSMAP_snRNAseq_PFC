...
metadata = metadata %>% mutate(nft_quartile_numeric =
                                 case_when(nft_quartile == "first_quarter" ~ 0, 
                                           nft_quartile == "second_quarter" ~ 1)
)
...
setwd("G:/PFC_429_final_Spring2021/Vanshika/muscat/Revision_Dec2022/nft_progression/Results_second_vs_first")
...
for (i in 1:length(tbl)){
  ...
  name=gsub("/","_",name)
  k1 <- tbl[[i]]
  write.csv(k1,file=paste0(name,"_","nft_quartile_numeric_second_vs_first",".csv"))
}
}
##############################################
